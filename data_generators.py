...
def citeseer_ego(dataset='citeseer', radius=3, node_min=50, node_max=400):
    _, G = graph_load(dataset)
...
def graph_ego(dataset='Digg', radius=3, node_min=50, node_max=100):
    with open(f"dataset/{dataset}_compressed.pkl", "rb") as f:
        G = pickle.load(f)
    G = max([G.subgraph(c) for c in nx.connected_components(G)], key=len)
    G = nx.convert_node_labels_to_integers(G)
    graphs = []
...
def generate_dataset(data_dir='data', dataset='community_small'):
    ...
    if dataset == 'community_small':
        ...
        graphs = citeseer_ego("citeseer",radius=1, node_min=4, node_max=18)[:200]
        save_dataset(data_dir, graphs, dataset)
        print(max([g.number_of_nodes() for g in graphs]))
    
    elif dataset == 'Digg':
        graphs = graph_ego("Digg",radius=2, node_min=30, node_max=100)
        save_dataset(data_dir, graphs, dataset)
        print(max([g.number_of_nodes() for g in graphs]))
...
def visualize_dataset(dataset='community_small'):
    # Load your dataset (update the path as needed)
    with open(f'data/{dataset}.pkl', 'rb') as f:
        graph_list = pickle.load(f)
    # Randomly select 16 graphs
    selected_graphs = random.sample(graph_list, 16)

    plot_graphs_list(graphs=selected_graphs, title=f'{dataset}', max_num=16,save_dir=f'data')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default='Digg', help='dataset to generate',
                        choices=['ego_small', 'community_small', 'ENZYMES', 'grid', 'Digg'])
